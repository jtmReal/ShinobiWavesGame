...
public class Weapon : MonoBehaviour 
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(0))//if player clicks left mouse button
        {
            ...
            {
                Instantiate(projectile, Camera.main.ScreenToWorldPoint(Input.mousePosition), Quaternion.identity);//projectile is spawned at shotPoint position with the rotation the weapon currently already has
